...
Groups are auto-created when you pass \`groupName\` to \`bookmark_add\`. To rename a group: \`group_rename\`. To move a bookmark to a different group: delete it with \`bookmark_delete\` and re-add it with \`bookmark_add\` using the new \`groupName\`. Groups can be reordered in the panel by drag-and-drop.
